...
def eval_policy(policy, eval_env, eval_episodes=5):
...
if __name__ == "__main__":
	...
	parser.add_argument("--env_name", default="reach-v2")          # OpenAI gym environment name
	...
	expl_env = utils.make_single_metaworld_env(args.env_name, seed=args.seed)
	eval_env = utils.make_single_metaworld_env(args.env_name, seed=args.seed)
	...
	state, done = expl_env.reset(), False
	episode_reward = 0
	episode_timesteps = 0
	episode_num = 0
	...
	for t in range(int(args.max_timesteps)):
		...
		# Perform action
		next_state, reward, done, _ = expl_env.step(action)
		done_bool = float(done) if episode_timesteps < expl_env.max_path_length else 0
